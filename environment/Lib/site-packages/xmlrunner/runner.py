...
class XMLTestRunner(TextTestRunner):
    """
    A tests runner class that outputs the results in JUnit like XML files.
    """
    ...
    def run(self, test):
        """
        Runs the given tests case or tests suite.
        """
        try:
            # Prepare the tests execution
            ...
            self.stream.writeln(result.separator2)
            run = result.testsRun
            self.stream.writeln("Ran %d tests%s in %.3fs" % (
